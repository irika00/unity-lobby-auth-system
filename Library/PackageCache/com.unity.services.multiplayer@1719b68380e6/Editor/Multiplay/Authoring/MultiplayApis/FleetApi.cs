...
namespace Unity.Services.Multiplay.Authoring.Editor.MultiplayApis
{
    class FleetApi : IFleetApi
    {
        ...
        public async Task<Fleet> GetFleetAsync(Guid fleetId, CancellationToken cancellationToken = default)
        {
            var request = new GetFleetRequest(m_ApiConfig.ProjectId, m_ApiConfig.EnvironmentId, fleetId);
            var response = await TryCatchRequestAsync(request, async(req) => await m_Client.GetFleetAsync(req));
