...
namespace Unity.Services.Multiplay.Authoring.Editor.MultiplayApis
{
    class ServersApi : IServersApi
    {
        ...
        public async Task<List<Server>> ListServersInFleetAsync(Guid fleetGuid, CancellationToken cancellationToken = default)
        {
            var request = new ListServersRequest(m_ApiConfig.ProjectId, m_ApiConfig.EnvironmentId, fleetId: fleetGuid);

            var response = await TryCatchRequestAsync(request, async(req) => await m_Client.ListServersAsync(req));
