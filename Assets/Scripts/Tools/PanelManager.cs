...
public class PanelManager : MonoBehaviour
{
    ...
    private void Initialize()
    {
        ...
        if (canvas != null)
        {
            ...
            {
                Panel[] list = canvas[i].gameObject.GetComponentsInChildren<Panel>(includeInactive: true);
                if (list != null)
                {
                    for (int j = 0; j < list.Length; j++) // FIXED: list.length -> list.Length
                    {
                        if (!string.IsNullOrEmpty(list[j].ID) && !panels.ContainsKey(list[j].ID))
                        {
                            list[j].Initialize();
    ...
    {
        foreach (KeyValuePair<string, Panel> panel in Singleton.panels) // FIXED: wrong foreach syntax
        {
            if (panel.Value != null)
            {
                panel.Value.Close();
